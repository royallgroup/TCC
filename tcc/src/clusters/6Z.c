...
int count_spindles_in_ring(const int *first_5A_cluster, const int *second_5A_cluster, int *spindle_1_id, int *spindle_2_id);

int other_count_spindles(const int *second_5A_cluster, const int *first_5A_cluster, int *s1b, int *s2b);
            ...
            for (j = 0; j < nmem_sp3c[first_5A_cluster[first_5A_spindle_id]]; j++) {
                ...
                if (second_5A_id > first_5A_id) {
                    ...
                    if (check_for_common_spindle_particles(first_5A_cluster, second_5A_cluster) == 0) {
                        ...
                        if (count_spindles_in_ring(first_5A_cluster, second_5A_cluster, &s1a, &s2a) == 1) {
                            if (count_spindles_in_ring(second_5A_cluster, first_5A_cluster, &s1b, &s2b) == 1) {

                                if (Bonds_BondCheck(s1a, s1b) == 1) {
...
int count_spindles_in_ring(const int *first_5A_cluster, const int *second_5A_cluster, int *spindle_1_id, int *spindle_2_id) {
    ...
    if (spindles_in_ring == 1) {
        (*spindle_1_id) = first_5A_cluster[3];
        (*spindle_2_id) = first_5A_cluster[4];
        return 1;
    }
    ...
    if (spindles_in_ring == 1) {
        (*spindle_1_id) = first_5A_cluster[4];
        (*spindle_2_id) = first_5A_cluster[3];
        return 1;
    }
    else return 0;
}
